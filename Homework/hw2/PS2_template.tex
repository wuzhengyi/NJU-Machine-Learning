...
\begin{enumerate}[(1)]
	...
	同书上p59的简写,对数似然为
	\begin{eqnarray*}
		\ell(\beta)&=&\sum^{K}_{n=1}{\ln{p(y=n|\mathbf{x})}}\\
		&=&\sum^{K-1}_{n=1}{\ln{\frac{e^{\mathbf{w}_n^\mathrm{T}\mathbf{x}+b_n}}{1+\sum^{K-1}_{i=1}e^{\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i}}}}+\ln{\frac{1}{1+\sum^{K-1}_{i=1}e^{\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i}}}\\
		&=&\sum^{K-1}_{i=1}{()\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i)} 
		+\sum^{K}_{i=1}{\ln(1+\sum^{K-1}_{i=1}e^{\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i})}
	\end{eqnarray*}

	\item 该对数似然的梯度为:
	$$\frac{\partial\ell(\beta)}{\partial x}=\sum^{K-1}_{i=1}{\mathbf{w}_i}+
		\sum^{K}_{i=1}{\frac{\mathbf{w}_ie^{\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i}}{1+\sum^{K-1}_{i=1}e^{\mathbf{w}_i^\mathrm{T}\mathbf{x}+b_i}}}$$
	\end{enumerate}
\end{solution}
\newpage
